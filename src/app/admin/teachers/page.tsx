...
export default function TeachersManagementPage() {
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(u => 
      ...
      u.specialty?.toLowerCase().includes(lowerSearch)
    );
  }, [users, searchTerm]);

  const handleDelete = async (userId: string) => {
    if (!firestore) return;
    if (!confirm("Are you sure you want to remove this teacher record?")) return;
    ...
    deleteDoc(docRef)
      .then(() => {
        toast({
          ...
          description: "The teacher record has been removed successfully.",
        });
      })
      .catch(async (error) => {
    ...
    <div className="space-y-8">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <Button asChild className="h-14 px-8 bg-gradient-to-r from-blue-600 to-teal-500 hover:from-teal-500 hover:to-blue-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/20 gap-2 text-base cursor-pointer border-none transition-all active:scale-95">
          <Link href="/admin/users/create?role=teacher">
            <UserPlus className="w-6 h-6" /> Add Teacher
          </Link>
        </Button>
      </div>
      ...
      {loading ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-gray-400">
          ...
          <p className="font-bold">Syncing Faculty Directory...</p>
        </div>
        ...
        <div className="text-center py-32 bg-white rounded-[32px] shadow-sm border border-dashed border-gray-200">
          <div className="w-20 h-20 bg-gray-50 rounded-full flex items-center justify-center mx-auto mb-6">
            <Contact2 className="w-10 h-10 text-gray-300" />
          </div>
          <h3 className="text-xl font-bold text-gray-900">No Teachers Found</h3>
          <p className="text-gray-500 mt-2 max-w-xs mx-auto">Create teacher accounts to populate your academy's faculty directory.</p>
          <Button asChild variant="outline" className="mt-8 rounded-xl font-bold">
            <Link href="/admin/users/create?role=teacher">Register First Teacher</Link>
          </Button>
        </div>
      ) : (
            ...
            <Card key={user.id} className="group border-none shadow-[0_10px_40px_rgba(0,0,0,0.04)] rounded-[24px] overflow-hidden bg-white hover:shadow-[0_20px_60px_rgba(0,0,0,0.08)] transition-all duration-500">
              ...
              <CardContent className="p-8">
                  ...
                  <DropdownMenu>
                    ...
                    <DropdownMenuContent align="end" className="w-44 rounded-xl shadow-xl border-gray-100 p-1">
                      ...
                      >
                        ...
                        <span className="font-bold text-xs">Remove Record</span>
                      </DropdownMenuItem>
                    </DropdownMenuContent>
                  </DropdownMenu>
                </div>
                ...
                <div className="space-y-4 pt-6 border-t border-gray-50">
                  ...
                  {user.bio && (
                    <div className="flex items-start gap-3 text-xs text-gray-400 font-normal text-left line-clamp-2 italic">
                      <BookMarked className="w-4 h-4 text-gray-200 mt-0.5" />
                      <span>{user.bio}</span>
                    </div>
                  )}
                </div>
